...
/* Trigger the OS screenshot handler. Writes /screenshotNNNN.bmp at the
   filesystem root when the iPod OS filesystem is mounted. This captures
   the OS compositor, not direct-MIPI homebrew app pixels. */
bool hb_screenshot_take(void);

/* Optional: write /appshotNNNN.bmp using the SDK's shadow copy of pixels
   drawn through hb_fill_rect / text / glyph APIs. Disabled by default;
   build with NANOAPPS_ENABLE_APP_SCREENSHOTS=1 to enable it. */
bool hb_app_screenshot_take(void);

/* Debounced Home+Power chord helper. Returns true when it fired. Apps
   with custom loops can call this once per frame; hb_ui_poll already
   calls it for hb_ui-based apps. The chord captures the homebrew app
   framebuffer, not the OS compositor. */
...
/* ---- Filesystem (hb_fs.c) ----
   ...
   filesystem mounted. Run `tools/eject.sh` once. While in "Connected"
...
/* Set the FAT attribute byte on a file/dir. Common bits:
   0x01=ReadOnly, 0x02=Hidden, 0x04=System, 0x10=Directory.
   The OS uses attr=1 to mark /iPod_Control hidden after mkdir.
...
/* ---- Settings (hb_settings.c) ----
   ...
   Persisted automatically — no flush call needed.
...
/* Settings key catalog. Each constant is the literal string the OS, all
   pulled from `strings`. Pass it as `key` to hb_settings_get_int/set_int/
   set_str. Type/range info is documented next to the definition in
   hb_settings.c. */
extern const char *const HB_PREF_GENERAL_BACKLIGHT_TIMER;
extern const char *const HB_PREF_GENERAL_BRIGHTNESS;
extern const char *const HB_PREF_GENERAL_CLICKER;
extern const char *const HB_PREF_GENERAL_FONT_SIZE;
extern const char *const HB_PREF_GENERAL_HOME_SCREEN;
extern const char *const HB_PREF_GENERAL_LANGUAGE;
extern const char *const HB_PREF_GENERAL_MUSIC_MENU;
extern const char *const HB_PREF_GENERAL_PREVIEW_PANEL;
extern const char *const HB_PREF_GENERAL_RADIO_LIVE_PAUSE;
extern const char *const HB_PREF_GENERAL_REQUIRE_PASSCODE;
extern const char *const HB_PREF_GENERAL_ROTATE;
extern const char *const HB_PREF_GENERAL_SLEEPWAKE_BUTTON;
extern const char *const HB_PREF_GENERAL_SLEEPWAKE_BTN_ENAB;
extern const char *const HB_PREF_GENERAL_SORT_CONTACTS;
extern const char *const HB_PREF_GENERAL_UI_ROTATION;
extern const char *const HB_PREF_GENERAL_WALLPAPER;
extern const char *const HB_PREF_TEMP_BACKLIGHT_TIMER;
extern const char *const HB_PREF_PLAYBACK_AIRPLANE_MODE;
extern const char *const HB_PREF_PLAYBACK_AUDIOBOOK_SPEED;
extern const char *const HB_PREF_PLAYBACK_COMPILATIONS;
extern const char *const HB_PREF_PLAYBACK_CROSSFADE;
extern const char *const HB_PREF_PLAYBACK_EQ;
extern const char *const HB_PREF_PLAYBACK_ENERGY_SAVER;
extern const char *const HB_PREF_PLAYBACK_HEADSET_EQ;
extern const char *const HB_PREF_PLAYBACK_REPEAT;
extern const char *const HB_PREF_PLAYBACK_SHAKE;
extern const char *const HB_PREF_PLAYBACK_SHUFFLE;
extern const char *const HB_PREF_PLAYBACK_SOUND_CHECK;
extern const char *const HB_PREF_PLAYBACK_USE_REC_VOL_RANGE;
extern const char *const HB_PREF_PLAYBACK_VOLUME_LIMIT;
extern const char *const HB_PREF_PHOTOS_MUSIC;
extern const char *const HB_PREF_PHOTOS_REPEAT;
extern const char *const HB_PREF_PHOTOS_SHUFFLE;
extern const char *const HB_PREF_PHOTOS_TIME_PER_SLIDE;
extern const char *const HB_PREF_PHOTOS_TRANSITIONS;
extern const char *const HB_PREF_BLUETOOTH_ENABLED;
extern const char *const HB_PREF_SETTINGS_SOFTWARE_VERSION;
extern const char *const HB_PREF_TRAINER_GENDER;
extern const char *const HB_PREF_TRAINER_HEIGHT;
extern const char *const HB_PREF_TRAINER_WEIGHT;
extern const char *const HB_PREF_TRAINER_BODY_LOCATION;
extern const char *const HB_PREF_TRAINER_DISTANCE_UNITS;
extern const char *const HB_PREF_TRAINER_MEASUREMENT_SYSTEM;
extern const char *const HB_PREF_TRAINER_PEDOMETER_STEP_GOAL;
extern const char *const HB_PREF_TRAINER_WEIGHT_UNITS;
extern const char *const HB_PREF_TRAINER_POWER_SONG;
extern const char *const HB_PREF_TRAINER_VOICE_FEEDBACK;
...
typedef struct {
    ...
    uint8_t  _pad;        /* +5  alignment for u16 below */
    uint16_t year;        /* +6 */
...
/* ---- App bundle loader (hb_app_loader.c) ----

   /Apps/ is the homebrew app directory. Each app is a `.app` folder:

       /Apps/Counter.app/
           Counter        — flat binary (linked at HB_APP_LOAD_VA)
           Name.txt       — optional UTF-8 display name (else dir name)
           Icon.bmp       — optional 24bpp BMP, max 64x64 (future)

   hb_app_scan() enumerates valid bundles. hb_app_load_and_exec()
   reads + jumps. Note: launching overwrites the calling app's code
   (same load VA), so the launcher cannot resume after launch — user
   re-enters via the homescreen Homebrew icon (or iTunes U hijack). */
typedef struct {
    char dir[64];     /* e.g. "Counter.app" */
    char label[64];   /* display name; defaults to dir-minus-.app */
    char exec[160];   /* full path: /Apps/Counter.app/Counter */
} hb_app_info_t;

/* Scan /Apps/, fill out[]. Returns count (0..max_apps). */
int  hb_app_scan(hb_app_info_t *out, int max_apps);

/* Read .bin into the standard load VA and jump. Returns false if
   read failed; otherwise does not return. Uses /Apps/.load_stub as
   a trampoline so the caller's .text isn't running while it gets
   overwritten. */
bool hb_app_load_and_exec(const char *path);

/* Direct (unsafe) load — for callers running OUTSIDE the standard
   load VA (i.e., from the load stub or shellcode region). Same
   logic as the original implementation. Don't call from apps. */
...
bool hb_media_is_video(void);         /* video mode? */
void hb_media_set_paused(bool paused);
void hb_media_next(void);
void hb_media_prev(void);
void hb_media_show_now_playing(void); /* opens the Now Playing OS screen */

/* ---- Battery (hb_battery.c) ----

   Read OS PMU cache fields. Layout was determined empirically from
   plugged/unplugged cache diffs and firmware call sites. */
...
/* ---- Helvetica (hb_helvetica.c) ----

   Real Helvetica via a pre-rendered bitmap font (sdk/generated/
   hb_font_helvetica.c). 16x15 glyph cell, proportional advance.
   Threshold-based rendering: glyph pixels >= 128 -> fg, else -> bg.

   Per-glyph: one MIPI set_window + bulk push (~1 long-packet/char).

   Caller must hb_ui_init() first (MIPI must be up). */
int16_t hb_helvetica_draw       (int16_t x, int16_t y,
                                 const char *s,
                                 hb_color_t fg, hb_color_t bg);
